...
use TYPO3\Surf\Encryption\InvalidPassphraseException;
use TYPO3\Surf\Encryption\KeyPair;
use TYPO3\Surf\Utility\Files;

/**
 * Encryption command controller
 */
class EncryptCommandController extends Command {
	...
	public function setupCommand($passphrase = NULL, $configurationPath = NULL) {
		...
		Files::createDirectoryRecursively($deploymentPath . '/Keys');
	...
	public function sealCommand($deploymentName = '', $configurationPath = NULL) {
		...
		$configurations = Files::readDirectoryRecursively($deploymentConfigurationPath, 'yaml');
	...
	public function openCommand($passphrase = NULL, $deploymentName = '', $configurationPath = NULL) {
		...
		} catch(InvalidPassphraseException $exception) {
			$this->outputLine('Local key is encrypted with passphrase. Wrong or no passphrase given.');
			$this->quit(1);
		}
		$configurations = Files::readDirectoryRecursively($deploymentConfigurationPath, 'yaml.encrypted');
		foreach ($configurations as $configuration) {
	 ...
	 * @return void
	 */
	protected function writeKeyPair(KeyPair $keyPair, $filename) {
	...
	protected function readKeyPair($filename) {
		...
		$keyPair = new KeyPair($data['privateKey'], $data['publicKey'], $data['encrypted']);
		return $keyPair;
	}
