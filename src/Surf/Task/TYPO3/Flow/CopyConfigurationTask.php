 ...
 * executing this task!
 */
class CopyConfigurationTask extends Task {
	...
	public function execute(Node $node, Application $application, Deployment $deployment, array $options = array()) {
		if (!isset($options['username']) && !$node->isLocalhost()) {
			throw new InvalidConfigurationException(sprintf('Missing "username" option for node "%s"', $node->getName()), 1348844231);
		}
		...
		$encryptedConfiguration = Files::readDirectoryRecursively($configurationPath, 'yaml.encrypted');
		if (count($encryptedConfiguration) > 0) {
			throw new TaskExecutionException('You have sealed configuration files, please open the configuration for "' . $deployment->getName() . '"', 1317229449);
		}
		$configurations = Files::readDirectoryRecursively($configurationPath, 'yaml');
		$commands = array();
