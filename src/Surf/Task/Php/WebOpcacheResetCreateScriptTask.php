 ...
 */
class WebOpcacheResetCreateScriptTask extends Task {
	...
	public function __construct() {
		$this->shellCommandService = new ShellCommandService();
	}
	...
	public function execute(Node $node, Application $application, Deployment $deployment, array $options = array()) {
		...
		if (!isset($options['scriptIdentifier'])) {
			...
			$scriptIdentifier = substr(str_shuffle('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'),0, 1) . substr(str_shuffle('aBcEeFgHiJkLmNoPqRstUvWxYz0123456789'),0, 31);
